...
const readIndexYml = () =>
  yaml.safeLoad(fs.readFileSync("source/index.yml", "utf8"));
const getPageData = () => {
  ...
  const getPageData = {
    ...
    langs: (config.language_tabs || []).map((lang) =>
      typeof lang === "string" ? lang : lang.keys.first
    ),
  };
  return getPageData;
};
...
const task = {
  ...
  runStaticAnalysis: () => {
    const ignoreDuplicateIds = (type, message) =>
      !/^Duplicate ID/.test(message);
    return mergeStream(
      gulp
  ...
  buildCss: () => {
    return gulp
      .src("source/stylesheets/*.css.scss")
  ...
  runServer: () => {
    ...
    gulp.watch(
      "source/index.yml",
      gulp.parallel("build-highlightjs", "build-js", "build-html")
    );
